...
mixin、trait、カテゴリー、プロトコル拡張など

- 継承を使用することなく、クラスやオブジェクトに新しい機能を追加したい
- 新しい機能を作成して、名前をつけてクラスやオブジェクトに追加する
- **機能の共有にはmixinを使用する**
...
- アプリケーションが本番稼働に入ったあとでもある程度挙動を変えられるようにすること
- 外部設定を用いてアプリをパラメータに対応させる
- 設定データに含めるもの
    - 外部サービスの認証情報（データベースや、サードパーティのAPI）
    - ログ出力レベルや、ログの出力先
    - アプリが依存するポート番号や、IPアドレス、マシン名、クラスター名
    - 環境依存の検証パラメーター
    - 税率などの外部で保持すべきパラメーター情報
    - サイト固有のフォーマット詳細データ
    - ライセンスキー
...
- プレインテキスト形式を使用する
    - YAMLとJSON
- 設定情報はアプリケーションの起動時に、データ構造としてアプリケーション内に渡される
- 通常の場合、大域変数として参照できるようになる
- そうではなく、設定情報はAPIの裏側に隠しておくようにするべき
- 設定の表現という詳細とコードを分離する

### サービスとしての設定

- 静的な設定は一般的ではあるが、我々は現在異なったアプローチを推奨
- サービスAPIの背後に保持するメリット
    - 複数のアプリケーションで設定情報を共有でき、認証やアクセス制御によってそれぞれが参照できる内容を制限できる
    - 設定の変更が大域的に適用できる
    - 設定データを専用のUI経由で管理できる
    - 設定データを動的に変更できる
- 設定データを動的に変更できるのはアプリケーションの利用可能性を向上させる上で非常に重要になる
- 設定値が変更された場合、コードを再構築する必要はない

### ドードー鳥なコードは記述しない

- 外部での設定に対応できる、適用性の高い、柔軟性の高いコードにする
