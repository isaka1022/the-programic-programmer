...
def text_warehouse():
    wh = Warehouse({"shoes": 10, "hats": 2, "unbrellas": 0})
    assert wh.in_stock("shoes")
    assert wh.in_stock("hats")
    assert not wh.in_stock("unbrellas")
...
def test_order_in_stock():
    wh = Warehouse({ "shoes": 10, "hats": 2, "unmrellas": 0})
...
def test_order_not_in_stock():
    wh = Warehouse({"shoes": 10, "hats": 2, "unbrellas": 0})
    status, item, quantity = order(wh, "unbrellas", 1)
    assert status == "not available"
    assert item == "unbrellas"
    assert quantity == 1
    assert wh.stock_count("unbrellas") == 0
...
# "hat"か"shoe"からパラメーターとしての品物をランダムに選択し、1~4個を選んでテストするようになっている
