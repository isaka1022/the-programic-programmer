...
- テスト対象コードが十分に正しいかどうかは判断できない
- カバレッジ分析ツールで包括的かどうかを知れる
    - 100%を目指すことを目的にしない
    - 行ではなくて、状態の数が重要
- **コードのカバレッジではなくて状態のカバレッジをテストする**

プロパティベースのテスト

- テスト対象のコードの契約と不変性に関するテストデータを生成させる

網の目を細かくする

- **同種のバグを一度に見つける**
- テスト担当者がバグを見つけたら、今後はそのバグを人間が見つけないようにする
...
- 開発者としての目的は「ユーザーを喜ばせる」こと
- ユーザーは目的と予算というコンテキストの中で手元にある業務の問題を解決しようとしている
- 彼らの期待を発掘するために
「プロジェクト完了後の1ヶ月（あるいは1年でも）が心安らかであったことを、どのようにすれば判断できるでしょうか？
- プロジェクトの背後にある期待を洗い出したのであれば、その期待に答える方法について考え初める
    - チームの全員がこういった期待について完全に認識しているかどうかを確認
    - 意思決定の際に、どの選択肢が期待に最も沿うのかを考える
    - 期待という観点からユーザーの要求に批判的な目を向ける
        - 多くのユーザーが提示した要求は、実際のところテクノロジーによって実現できると類推されたものごとでしかなかった
    - プロジェクトの進展とともにこういった期待について考え続ける
- 業務領域の知識が増えるとともに、根本となる業務上の問題に取り組めるような示唆ができるようになっている
...
- 責任逃れを潔しとしない
- **あなたの作品に署名すること**
- 複数の人々で構成されると問題を引き起こす可能性がある
    - 自らの責任範囲に固執
    - 自らのコードに対して傲慢になる
- コードは所有される必要があるが、個人が所有する必要はない
- 記述したコードと仕事について責任を持つこと
